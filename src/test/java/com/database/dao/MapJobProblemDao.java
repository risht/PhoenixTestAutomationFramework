		...
		try {
			...
			while(rs.next())
			{
				...
				mapJobProblemModel = new MapJobProblemModel(
		                rs.getInt("id"),              // maps to id
		                rs.getInt("tr_job_head_id"),  // maps to tr_job_head_id
		                rs.getInt("mst_problem_id"),  // maps to mst_problem_id
		                rs.getString("remark"));           // maps to remark

			}
			
		} catch (SQLException e) {
			LOGGER.error("Cannot convert the resultset to mapJobProblemModel bean",e); 
